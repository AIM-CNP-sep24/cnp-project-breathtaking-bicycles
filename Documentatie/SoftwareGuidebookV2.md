...
De belangrijkste gebruikers van de applicatie zijn de zorgvrager, de zorgverlener en de beheerder. Voor zorgvrager en zorgverlener biedt het systeem een manier om ondanks taalverschillen met elkaar te kunnen communiceren, met name via de vertaalpagina en de pagina voor standaardbenodigdheden. De beheerder heeft op zijn beurt de mogelijkheid om de structuur en inhoud van die pagina’s aan te passen via het beheerscherm, zodat de applicatie actueel en relevant blijft voor de praktijk.
...
### Standaard benodigdheden
Als eerste de standaardbenodigdheden, het doel hiervan is dat zorgvragers makkelijk kunnen navigeren naar benodigdheden die ze op een reguliere basis nodig hebben, zoals eten & drinken en naar de wc gaan. Door op de grote iconen te klikken wordt de naam van de benodigdheid uitgesproken op de speakers, in de taal van de zorgverlener, zodat deze weet wat de zorgvrager graag wilt hebben. 
...
### Inloggen
Als laatste grote functionaliteit hebben we een inlogpagina waarmee, met behulp van correcte inloggegevens, toegang verleend kan worden tot beide instelmenu's. Dit is nodig om te voorkomen dat de zorgvrager perongeluk op een instelmenu klikt, en instellingen aanpast. Het is mogelijk om hier in te loggen als zorgverlener en systeembeheerder. 
...
Voor de login functionaliteit is spring security gebruikt. De belangrijkste klassen hiervoor zijn de SecurityConfig, JwtUtil en de DatabaseUserDetailService. De Config klasse zorgt ervoor dat de login goed is ingesteld en de beveiliging binnen de applicatie gewaarborgd wordt. De klasse zorgt er dus voor dat niet alle pagina's binnen de applicatie direct bereikbaar zijn, maar dus beveiligd zijn met een loginfunctie.
De DatabaseUserDetailService is een klasse die de gebruikers uit de database ophaalt en hiervan een spring security user object maakt. 
...
De belangrijkste folders binnen de applicatie zijn de .github, backend en frontend folders. Binnen deze folders bevindt zich de code die door het project heen geschreven is met de daarbij horende testimplementaties. De tests voor de backend zijn te vinden in het folder "test" en de tests voor de frontend zijn te vinden in het folder "stories".
 ...
 In het .github folder staan de workflows die betrekking hebben tot de pipeline die in gebruik is geweest binnen de looptijd van dit project. De pipeline wordt verder besproken in het hoofdstuk Integratie.


## Gebruikte Libraries en tools
De backend van de applicatie, opgebouwd met behulp van Maven, is geschreven in Java met behulp van het Springboot framework. Springboot is een open source Java-framework waarmee applicaties gebouwd en gerund kunnen worden zonder hulp van een externe webserver. Voor de de login is de customizable authentication and access-control Spring Security framework toegepast. De authenticatie en authorisatie zijn door middel van dit framework verwerkt in de applicatie, zodat niet alle pagina's unauthorized bereikbaar zijn voor de gebruiker. 
...
## Andere data
Naast de database wordt ook gebruik gemaakt van de LibreTranslate API. ingevoerde tekst kan naar deze API verstuurd worden en wordt vervolgens vertaald naar de gewenste taal. Vervolgens geeft deze API de vertaling van de verstuurde tekst weer terug.
...
Onze CI-pipeline draait automatisch bij elk **pull request** via GitHub Actions. De pipeline is verdeeld in drie jobs:
